...
## 📍 いまのフェーズ（2026-07-09 更新）

| 項目 | 現在 |
|---|---|
| **いまの段階** | [§7](#7-進め方提出までの5ステップ) の **Step 1（Basic ベースライン）＋ Step 2（sweep 参加）を並行中** |
...
| **各自やること** | Kaggle で公式 sweep に参加する（[§5](#5-kaggleコミットでの-sweep-の回し方)）＋ 手元で `classic_baseline.yaml` を自分の seed で回す（[§7](#7-進め方提出までの5ステップ) Step 1） |
