...
**（追記）** 実際にはこの「やり直し」を待つ前に、同じ4周目の設定で並行して走っていた別のセッションが NaN クラッシュせず完走していたことが判明した。詳細は [§9](#9-追記2026-07-16-light-hill-20-の完走とresume先の更新)。
...
## 9. 追記（2026-07-16）: light-hill-20 の完走と resume先の更新

**背景:** §8 で NaN クラッシュした `jp5jwddi`（mild-lion-18）と同じ4周目の設定（`pf8e9dqb` から vel_coef=0 で+100万step）で、並行してもう1本のセッションが走っていた。こちらは run `75glj5ue`（light-hill-20、2026-07-15開始・runtime約10時間・global_step 4,000,000）としてクラッシュせず完走した。§8 の対策（use_expln 等）が入る前の run だが、たまたま発散せずに終えられた。
...
- §8 の「同じ周をやり直す」計画は不要と判断し、`configs/hardcore_next_run.yaml` の `resume_run_path` を `pf8e9dqb` → `75glj5ue` に更新（Hardcore完走率55%→65%への改善を受けて）。`config_path` は vel_coef=0 のまま継続（完走率にまだ伸びしろがあるため）。
...
- §8 の NaN 対策（use_expln・安全停止・ベスト即アップロード）は次周の run から有効になるため、今後クラッシュが起きてもベストモデルの回収は可能になっているはず。
