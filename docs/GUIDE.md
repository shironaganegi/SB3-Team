...
5. **最後のセルの変数2つを確認する。** ノートの一番下のセルの冒頭に
   ...
   という2つの変数があります。編集するのはここだけです。`SWEEP_ID` は普段は既定値のままでよく、別の sweep に参加するときだけ手順1のIDに貼り替えます。`N_RUNS` は回す本数で、CPUなので 3〜5 程度が目安です。
   - *なぜ:* このセルが「司令塔（sweep）から設定をもらって学習を `N_RUNS` 本回せ」という今回の仕事の本体です。本数を控えめにするのは、1本の学習が数時間かかるため、欲張るとセッションの時間上限（約12時間）に達して途中で切られるからです。途中で切られても完了済みの run の結果は W&B に残っていますが、実行中だった1本は無駄になります。なお `SWEEP_ID` を空にしたまま実行すると、日本語のエラーメッセージで止まって知らせてくれます。
