...
ここから先（仮想環境の作成と依存インストール）は [メイン README の §3「セットアップ」](../README.md#3-セットアップ) に手順があります。要点だけ書くと、
...
「実験したいけど、どのファイルを触ればいいか分からない」というときは、[メイン README の §4.5「実験の地図」](../README.md#45-実験の地図何を変えたいときどこを編集するか) に「やりたいこと別の編集場所」の一覧表があります。迷ったらまずそこを見てください。
...
（いまの段階では baseline 専念の `baseline_sweep.yaml` を使います。`classic_sweep.yaml` との使い分けは [メイン README の §4.5 表B](../README.md#45-実験の地図何を変えたいときどこを編集するか) を参照。）
