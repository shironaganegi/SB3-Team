...
「実験したいけど、どのファイルを触ればいいか分からない」というときは、[メイン README の §4.5「実験の地図」](../README.md) に「やりたいこと別の編集場所」の一覧表があります。迷ったらまずそこを見てください。
...
（いまの段階では baseline 専念の `baseline_sweep.yaml` を使います。`classic_sweep.yaml` との使い分けは [メイン README の §4.5 表B](../README.md) を参照。）
