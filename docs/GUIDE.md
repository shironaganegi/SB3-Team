...
そのうえで、ノート内の Secrets（後述の W&B キー）を登録し、上のセルから順に実行 → 最後に「**Save & Run All（Commit）**」を押すと、セッションを閉じてもクラウド側で最後まで走り切ります。これが「放置実行」です。

このリポジトリは Public なので、Kaggle から clone する際に GitHub のトークンは不要です（ノートの clone セルはトークン無しの素のURLを使う作りになっています）。
